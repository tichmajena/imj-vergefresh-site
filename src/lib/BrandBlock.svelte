...
<div
	class="story-block layout-{block.layout} accented2 container mx-auto w-full"
	data-path={path.join('.')}
	data-type="block"
...
>
	<div
		...
		class="vt non-text-content aspect-video overflow-hidden rounded-xl bg-white"
	...
	<div class="caption">
		...
		<div class=" hidden space-x-3">
			{#if block?.socials?.instagram}
				<a
					class="btn btn-circle btn-ghost"
					aria-label="instagram"
					href={block.socials.instagram}
					target="_blank"
				>
					<IconInstagram />
				</a>
			{/if}
			{#if block?.socials?.youtube}
				<a aria-label="youtube" href={block.socials.youtube} target="_blank">
					<IconYoutube />
				</a>
			{/if}
			{#if block?.socials?.facebook}
				<a aria-label="facebook" href={block.socials.facebook} target="_blank">
					<IconFacebook />
				</a>
			{/if}
		</div>
...
<style>
	...
	.accented2 {
		@apply rounded-xl bg-[--accented2];
	}

	:global(.drawer[data-theme='light']) .accented2 {
		@apply px-8;
	}
</style>
