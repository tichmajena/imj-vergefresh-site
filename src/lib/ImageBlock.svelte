...
<div
	class="story-block layout-1 {block.classes} container mx-auto flex h-full w-full"
	data-path={path.join('.')}
	data-type="block"
	data-index={path.at(-1)}
	style="anchor-name: --{path.join('-')};"
>
	<div class="non-text-content relative min-h-screen w-1/6 overflow-hidden" contenteditable="false">
		...
		/>
		<div
			class="absolute top-0 right-0 bottom-0 left-0 flex items-start justify-center bg-black/60 pt-10 text-white inset-shadow-sm inset-shadow-black"
		>
			<Icon class="mb-4 w-16"></Icon>
		</div>
	</div>

	<div class="caption w-5/6 p-12">
		<!-- ATTENTION: Do not format the following lines, as whitespace will mess up contenteditable -->
		<Text class="heading2 text-primary" path={[...path, 'title']} {editable} />
